...
if [ -z "$RUN_ID" ]; then
    ...
    log "Polling every 10s for completion (capturing raw JSON each time)..."

    poll_count=0
    while true; do
        # Capture full raw JSON (no truncation) using gh api
        ...
        # Append the raw JSON with a timestamp marker
